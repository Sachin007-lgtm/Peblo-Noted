...
export default function OnboardingOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-4 flex flex-col md:flex-row items-stretch rounded-[40px] bg-[#07130a]/70 backdrop-blur-3xl border border-white/20 shadow-[0_0_100px_rgba(76,175,80,0.15)] overflow-hidden animate-scale-up pointer-events-auto">
        ...
        <div className="flex-1 p-10 md:p-14 flex flex-col justify-center relative">
          ...
          <h2 className="text-white text-3xl md:text-4xl font-extrabold tracking-tight leading-tight mb-4 drop-shadow-sm transition-all duration-500">
            {currentCard.title}
          </h2>
          
          <p className="text-gray-400 text-[15px] md:text-[17px] leading-relaxed mb-10 font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 mt-auto">
            <button 
              onClick={handleNext}
              className="w-full sm:w-auto bg-white text-black px-8 py-3 rounded-full font-bold text-[14px] hover:scale-105 active:scale-95 transition-all shadow-lg hover:shadow-white/20 flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              className="w-full sm:w-auto text-gray-500 hover:text-white px-6 py-3 font-semibold text-[14px] transition-colors flex items-center justify-center"
            >
              Skip Tour
            </button>
          </div>
            ...
            {CARDS.map((_, idx) => (
              <div 
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${
                  idx === currentIndex ? 'w-8 bg-accent' : 'w-2 bg-white/20'
                }`}
              />
            ))}
          </div>
        ...
        <div className="flex-1 w-full min-h-[300px] md:min-h-0 relative flex items-center justify-center border-t md:border-t-0 md:border-l border-white/10 bg-black/20 perspective-1000">
            
          {/* Deep blurred orb behind icon */}
          <div className={`absolute inset-0 m-auto w-64 h-64 rounded-full bg-gradient-to-br ${themeGradient} blur-[80px] opacity-40 transition-all duration-1000 ease-in-out`} />
          ...
          <div className="relative z-20 flex items-center justify-center transition-all duration-700 ease-out animate-float">
            <span 
              ...
              className="material-symbols-outlined text-[160px] md:text-[180px] text-transparent bg-clip-text bg-gradient-to-br from-green-300 via-accent to-emerald-800 drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)] animate-fade-up"
